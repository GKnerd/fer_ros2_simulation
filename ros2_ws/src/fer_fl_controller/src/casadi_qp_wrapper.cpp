...
cs::MX v = cs::MX::sym("v", 7);// each v is the virtual acceleration for one joint, so v is a vector of 7 virtual accelerations for the 7 joints of the robot. These are the decision variables of the optimization problem, and the solver will find the optimal values of v that minimize the cost function while satisfying the constraints
  ...
  const double dt    = 0.01; // 10 ms control timestep(prediction step)
  ...
  cs::MX cost = cs::MX(0.0);// Initialize the cost to zero, and we will add terms to it for each time step in the prediction horizon. The cost function includes terms for tracking the reference trajectory (position and velocity errors) at each time step, as well as a control rate penalty that encourages smooth changes in acceleration.
  for (int k = 1; k <= N; ++k) {
    const double t  = k * dt;// Time at the k-th prediction step (at k=1, t is 10 ms; at k=15, t is 150 ms). 
    const double t2 = t * t;

    cs::MX a_k = E_q + E_dq * t + 0.5 * ddq_ref * t2;// this builds the reference prediction position term at the k-th step based on the initial position error E_q, the initial velocity error E_dq, and the reference acceleration ddq_ref. The formula is derived from a second-order Taylor expansion of the reference trajectory around the current time, assuming constant acceleration over the prediction horizon. This gives us a predicted position error at time t that we want to minimize in the cost function.
    cs::MX b_k = E_dq + ddq_ref * t;

    cs::MX e_q  = a_k - 0.5 * t2 * v; // the position error at the k-th step is the predicted position error a_k minus a term that depends on the virtual acceleration v. The term 0.5 * t2 * v comes from integrating the acceleration v twice over time to get a position change, and it represents how the decision variable v will affect the position at time t. By including this in the cost function, we are effectively telling the solver to choose v in a way that minimizes the predicted position error at each step of the horizon.
    cs::MX e_dq = b_k - t  * v;
