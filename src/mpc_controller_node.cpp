...
private:
    ...
    double v_prev   = 0.0)
  {
    ...
    // u_abs_weight penalises the absolute magnitude of v (not just changes).
    // Without it, when q_des = 0 and q ≈ 0 the tracking terms collapse to
    // zero but v_prev keeps v* non-zero → overshoot past zero.
    // Adding u_abs_weight to the denominator biases v* toward zero whenever
    // the tracking error is small, eliminating the zero-joint overshoot.
    const double u_abs_weight       = 0.02;
    ...
    //   q_sim(k)  = q  + dq*(k+1)*dt + 0.5*v*((k+1)*dt)²
    //   dq_sim(k) = dq + v*(k+1)*dt
    //
    // Tracking errors (linear in v):
    //   e_k  = a_k - 0.5*v*τ²,  a_k = (q_ref−q) + (dq_ref−dq)*τ + 0.5*ddq_ref*τ²
    //   de_k = b_k - v*τ,        b_k = (dq_ref−dq) + ddq_ref*τ
    //
    // Cost: J = Σ[wq*e²+wdq*de²] + N*u_w*(v−v_prev)² + N*u_abs*v²
    // Setting dJ/dv = 0:
    //   num = Σ[wq*0.5*τ²*a_k + wdq*τ*b_k] + N*u_w*v_prev
    //   den = Σ[wq*0.25*τ⁴ + wdq*τ²]       + N*(u_w + u_abs)   ← u_abs added here
  ...
  void jointCallback(const sensor_msgs::msg::JointState::SharedPtr msg)
  {
    ...
    // Sequence: A -> B -> C -> D -> E -> home, then hold at home.
    if (t >= traj_duration_ && current_waypoint_ + 1 < waypoints_.size()) {
      ...
      RCLCPP_INFO(this->get_logger(),
        "Waypoint %zu/%zu", current_waypoint_ + 1, waypoints_.size());
    }
